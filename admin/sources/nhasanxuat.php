...
function save_item(){
	global $d;
	$file_name=changeTitle($_POST['ten_vi']).'-'.rand(0,1000);
	...
	if($id){
		...
		if($photo = upload_image("file", 'jpg|png|gif|JPG|jpeg|JPEG', _upload_product,$file_name)){
			$data['photo'] = $photo;	
			$data['thumb'] = create_thumb($data['photo'], 160, 82, _upload_product,$file_name,1);		
		...
		$d->setWhere('id', $id);
		if($d->update($data))
			redirect("index.php?com=nhasanxuat&act=man&page=".$_REQUEST['page']."&type=".$_GET['type']);
		else
