...
function get_main_list()
{
	...
	$str='
	<select id="id_list" name="id_list" class="main_select select2 select_danhmuc">
...
<div class="wrapper">
	...
	<form name="supplier" id="validate" class="form" action="index.php?com=product&act=save_cat<?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" method="post" enctype="multipart/form-data">
		<div class="widget">
				...
				<?php if($_GET['act']=='edit_cat'){?>
					<div class="formRow">
						...
						<div class="formRight">
							...
							<div class="mt10">
								<?php if (!$config_auto) { ?>
									<img class="img-responsive" src="../thumb/<?php echo _width_thumb.'x'._height_thumb.'/1/'._upload_product_l.$item['thumb']?>" style="max-height: 300px"  alt="NO PHOTO" />
								<?php } else { ?>
									<img class="img-responsive" src="<?php echo _upload_product.$item['photo']?>" style="max-height: 300px"  alt="NO PHOTO" />
								<?php } ?>
			...
			<?php } ?>
			<?php if($config_mul){ 
				$ds_photo = _result_array("SELECT id,stt,photo FROM table_product_photo WHERE type='".$_GET['type']."' AND id_cat='".@$item['id']."'");
				?>
				<div class="formRow">
					...
					<div class="formRight">
						...
						<div class="note"> width : <?php echo _width_thumb_mul*$ratio__mul;?> px , Height : <?php echo _height_thumb_mul*$ratio__mul;?> px </div>  
						<div class="clear"></div>            
						<?php if($act=='edit_cat'){?>
							<?php if(count($ds_photo)!=0){?>       
								<?php foreach($ds_photo as $value){?>
									<div class="item_trich">
										<img class="img_trich" width="140px" height="110px" src="<?=_upload_product.$value['photo']?>" />
										<input type="text" rel="<?=$value['id']?>" value="<?=$value['stt']?>" class="update_stt tipS" />
										<a class="delete_images" title="<?=$value['id']?>"><img src="images/delete.png"></a>
									</div>
								<?php } ?>
