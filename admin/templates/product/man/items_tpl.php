<script type="text/javascript">
  ...
  {
    var a=document.getElementById("id_list"), b=document.getElementById("id_cat"), 
    
    _e=document.getElementById("id_nsx");
    window.location ="index.php?com=product&act=man&type=<?=$_GET['type']?>&id_list="+a.value+"&id_cat="+b.value+"&id_nsx="+_e.value; 
    return true;
  }
    ...
    <thead>
      <tr>
        ...
        <?php if($config_nhasanxuat){ ?> 
          <td class="tb_data_small"><?=get_member()?></td>
        <?php } ?>
        ...
        <td class="tb_data_small" style="min-width: 200px;"></td>
        <?php if($config_sale){ ?>
        ...
        <?php } ?>
        <?php if($config_km){ ?>
          <td class="tb_data_small"><?php echo $config_text_km ?></td>  
        <?php } if($config_ts){ ?>
          <td class="tb_data_small"><?php echo $config_text_ts ?></td>       
        <?php } if($config_bc){ ?>     
          <td class="tb_data_small"><?php echo $config_text_bc ?></td>       
        <?php } if($config_noibat){ ?>     
          <td class="tb_data_small">Nổi bật</td>       
        <?php } ?>
        <td class="tb_data_small">Ẩn/Hiện</td>
        <td width="100" class="cell_control">Thao tác</td>
      </tr>
    </thead>
     ...
     <?php for($i=0, $count=count($items); $i<$count; $i++){?>
       <tr>
        ...
        <td class="title_name_data" style="width: 70px; text-align: center;">
          <a href="index.php?com=product&act=edit&id_list=<?=$items[$i]['id_list']?>&id_cat=<?=$items[$i]['id_cat']?>&id_item=<?=$items[$i]['id_item']?>&id_sub=<?=$items[$i]['id_sub']?>&id=<?=$items[$i]['id']?><?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" class="tipS SC_bold"><img src="<?php echo _upload_product.$items[$i]['photo'] ?>" alt="" style="max-width: 75px"></a>
        </td>
        <?php if( $config_images2 ) { ?>
          <td class="title_name_data" style="width: 70px; text-align: center;">
          <a href="index.php?com=product&act=edit&id_list=<?=$items[$i]['id_list']?>&id_cat=<?=$items[$i]['id_cat']?>&id_item=<?=$items[$i]['id_item']?>&id_sub=<?=$items[$i]['id_sub']?>&id=<?=$items[$i]['id']?><?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" class="tipS SC_bold"><img src="<?php echo _upload_product.$items[$i]['photo_2'] ?>" style="max-width: 75px" alt=""></a>
        </td>
        <?php } ?>
         <td class="title_name_data" style="width:200px; text-align: center;">
          <a href="index.php?com=product&act=edit&id_list=<?=$items[$i]['id_list']?>&id_cat=<?=$items[$i]['id_cat']?>&id_item=<?=$items[$i]['id_item']?>&id_sub=<?=$items[$i]['id_sub']?>&id=<?=$items[$i]['id']?><?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" class="tipS SC_bold"><?php echo $items[$i]['ten_vi'] ?></a>
        </td>
        <?php if($config_sale){ ?>
          <td align="center">
            <a data-val2="table_<?=$_GET['com']?>" rel="<?=$items[$i]['sp_uudai']?>" data-val3="sp_uudai" class="diamondToggle <?=($items[$i]['sp_uudai']==1)?"diamondToggleOff":""?>" data-val0="<?=$items[$i]['id']?>" ></a> 
          </td>
        <?php } ?>
        <?php if($config_km){ ?>
          <td align="center">
        ...
        <td class="actBtns cell_control" width="100">
